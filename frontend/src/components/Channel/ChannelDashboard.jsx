...
function ChannelDashboard({
  ...
  const updateMOSummary = (entries) => {
    ...
    const totalReq = entries.reduce((sum, item) => sum + Number(item.requiredQty || 0), 0);
    const totalProduced = entries.reduce((sum, item) => sum + Number(item.producedQty || 0), 0);
    ...
    <>
      <div className="channel-dashboard">
        <div className="channel-page-header">
          <h1>CHANNEL ENTRY</h1>
          <button className="new-mo-btn" onClick={handleNewMO}>
            + New MO
          </button>
        </div>
          ...
          <div className="form-grid">
            ...
            <div className="form-group">
              ...
              <input
                ...
                onChange={(e) => setFormData({ ...formData, packCode: e.target.value })}
              />
            </div>
            ...
            <tbody>
              {channelData && channelData.length > 0 ? (
                channelData.map((item) => (
                  <tr
                    key={item.id}
                    onClick={() => setSelectedMO(item)}
                    style={{
                      cursor: "pointer",
                      backgroundColor: selectedMO?.id === item.id ? "#E8F0FE" : "transparent",
                    }}
                  >
                    <td>{item.mo}</td>
        ...
        <div className="selected-card">
          <h2>Selected MO : {selectedMO ? selectedMO.mo : "--"}</h2>
          <div className="selected-actions">
            <button
              ...
              onClick={() => {
                ...
                setSelectedProduction(
                  productionEntries.find(entry => entry.mo === selectedMO?.mo)?.packCode || ""
                );
                setProductionQty("");
            ...
            <button className="edit-entry-btn" onClick={() => setEditMode(!editMode)}>
          ...
          <table className="production-table">
            ...
            <tbody>
              {productionEntries.length === 0 ? (
                <tr>
                  <td colSpan="5">Click an MO row in the summary table above to see its variants here.</td>
                </tr>
              ) : (
                productionEntries
                    ...
                    <tr key={entry.id || index}>
                      ...
                      <td>
                        <button
                          ...
                          onClick={() => {
                            const updated = productionEntries.filter((_, i) => i !== index);
                            setProductionEntries(updated);
                            updateMOSummary(updated);
                          }}
                        >
      ...
      {showProductionModal && (
        <div className="production-modal-overlay">
          <div className="production-modal">
            ...
            >
              <option value={selectedProduction}>{selectedProduction}</option>
            </select>
            ...
            <div className="modal-buttons">
              <button onClick={() => setShowProductionModal(false)}>Cancel</button>
              <button
                onClick={() => {
                  const updated = [...productionEntries];
                  if (updated[0]) {
                    updated[0] = {
                      ...updated[0],
                      producedQty: Number(updated[0].producedQty || 0) + Number(productionQty)
                    };
                    setProductionEntries(updated);

                    const updatedSummary = channelData.map((item) => {
                      if (item.id !== selectedMO.id) return item;
                      return { ...item, qty_sent: updated[0].producedQty };
                    });

                    updateChannelData(updatedSummary);
                    setSelectedMO(updatedSummary.find((item) => item.id === selectedMO.id));
                  }
